...
- Repeated snapshots with the same options and unchanged presented content and bounds return a compact acknowledgement. Use `--force-full` to re-emit the tree; `--json` and `--raw` retain full output.
